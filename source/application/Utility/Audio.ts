...
export default class Audio
{
	...
	{
		let withSound = (action:(...args:any[]) => void) =>
		{
			let that = this;
		...
		let updatePosition = (position:number) =>
		{
		...
		options.onload = withSound((s: boolean) =>
		{
			if (s)
				...
				Notification.Error("Error playing audio");
		});
	...
	private static UpdateRTMPOptions(options:ISoundOptions):void
	{
		...
		{
			let divide = options.url.lastIndexOf("mp3:");
